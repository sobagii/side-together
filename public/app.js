...
function setAuthMode(mode) {
  ...
  $("#authTitle").textContent = registering ? "팀 공간에 합류하기" : "팀 공간에 들어가기";
  $("#authDescription").textContent = registering
    ? "팀에게 받은 초대 코드로 계정을 만드세요."
    : "함께 만들고 있는 프로젝트로 돌아오세요.";
...
function listCardMarkup(post, index = 0) {
  ...
  return `
    <article class="post-card category-card" data-post-id="${post.id}">
      ${
...
async function deletePost(postId) {
  ...
  try {
    ...
    if (state.selectedIdeaId === postId) detailDialog.close();
    await loadWorkspace({ quiet: true });
...
$("#detailEditButton").addEventListener("click", () => {
  detailDialog.close();
  openEditDialog(state.selectedIdeaId);
});
